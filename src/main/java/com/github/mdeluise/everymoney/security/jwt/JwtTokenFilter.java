...
import com.github.mdeluise.everymoney.exception.ErrorMessage;
...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    @Autowired
    public JwtTokenFilter(JwtTokenUtil jwtTokenUtil, JwtWebUtil jwtWebUtil,
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (hasAccessToken(request)) {
            String token = jwtWebUtil.getAccessToken(request);
    ...
    private boolean hasAccessToken(HttpServletRequest request) {
        return jwtWebUtil.getAccessToken(request) != null;
    }
